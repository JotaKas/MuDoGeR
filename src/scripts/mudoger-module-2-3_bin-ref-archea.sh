...
########## PROKARYOTIC REFINEMENT FOR ARCHAEA (40,30) WITHOUT CONCOCT ###################
echo '------- START MODULE 2-3 BIN REFINEMENT FOR ARCHAEA (NO CONCOCT)'
...
log_info "Starting archaeal bin refinement without CONCOCT"
...
log_info "Running metaWRAP bin refinement for archaea (completeness ≥40%, contamination ≤30%)"
...
if metawrap bin_refinement -o "$output_folder" -t "$cores" \
    ...
    -c 40 -x 30 -m "$memory"; then

    log_success "Archaeal bin refinement completed successfully"

    # Verify output
    stats_file="$output_folder/metawrap_40_30_bins.stats"
    refined_bins_dir="$output_folder/metawrap_40_30_bins"
    ...
    if [[ -f "$stats_file" ]]; then
        ...
        if [[ -d "$refined_bins_dir" ]]; then
            ...
            log_success "Archaeal refinement produced $refined_count refined bins"

            if [[ $refined_count -eq 0 ]]; then
                log_warning "No refined bins met the quality criteria (≥40% completeness, ≤30% contamination)"
            fi
        else
...
log_success "Archaeal bin refinement module completed successfully"